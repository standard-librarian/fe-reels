...
const SIZES = {
  ...
  md: { circle: 'w-[46px] h-[46px]', icon: 21 },
}

const BUTTON =
  'flex flex-col items-center gap-1 border-0 bg-none ' +
  'text-white [text-shadow:0_1px_4px_rgba(0,0,0,0.6)] active:[&_.action-circle]:scale-[0.92]'

const CIRCLE =
  'action-circle grid place-items-center rounded-full ' +
  'transition-[transform,background-color,color,box-shadow] duration-[160ms] ease-out'
...
export function IconButton({ icon: Icon, label, onClick, active = false, pending = false, primary = false, size = 'md', showLabel = true, className = '' }: IconButtonProps) {
  ...
  const circleClass = [CIRCLE, circle, primary ? PRIMARY : GLASS, active && ACTIVE, pop && POP].filter(Boolean).join(' ')
  ...
  return (
    <button className={`${BUTTON} ${className}`} onClick={handleClick} aria-label={label} aria-busy={pending}>
      ...
      {showLabel && <span className="text-[11px] font-semibold">{label}</span>}
    </button>
  )
}
