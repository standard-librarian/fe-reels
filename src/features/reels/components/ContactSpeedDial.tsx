...
export function ContactSpeedDial({ open, phone, onToggle, onClose, onChat, onCallSheet }: ContactSpeedDialProps) {
  ...
  return (
    <div ref={wrapRef} className="relative flex flex-col items-center gap-3">
      <div
        className={`absolute bottom-[calc(100%+10px)] right-0 w-[142px] p-1.5 rounded-[18px] bg-[rgba(13,20,35,0.88)] border border-white/15 backdrop-blur-[18px] shadow-[0_18px_50px_rgba(0,0,0,0.38)] origin-bottom-right transition-[transform,opacity] duration-[250ms] ease-[cubic-bezier(0.2,0.85,0.2,1)] ${
        ...
        {options.map(option => (
          <button
            ...
            role="menuitem"
            tabIndex={open ? 0 : -1}
            className="w-full flex items-center gap-2.5 border-0 bg-transparent text-white p-1.5 rounded-[14px] text-left cursor-pointer transition-[background-color,transform] duration-[180ms] hover:bg-white/9 active:scale-[0.97]"
            onClick={() => {
              onClose()
              option.onPick()
            }}>
            <span className={`w-8 h-8 shrink-0 grid place-items-center rounded-full border [&_svg]:w-[17px] [&_svg]:h-[17px] ${option.iconClass}`}>{option.icon}</span>
            <strong className="text-[13px] font-bold">{option.title}</strong>
          </button>
        ))}
      </div>
